...
void main() {
    ...
    // Adicionar event listeners no container e na window para melhor captura
    if (mouseInteraction && containerRef.current) {
      containerRef.current.addEventListener('mousemove', handleMouseMove)
      containerRef.current.addEventListener('mouseenter', handleMouseEnter)
      containerRef.current.addEventListener('mouseleave', handleMouseLeave)
      // Também adicionar na window para capturar mesmo quando o mouse está sobre outros elementos
      window.addEventListener('mousemove', handleMouseMoveWindow)
    }
    
    // Delay para garantir que o container tenha dimensões
    setTimeout(() => {
      resize()
    }, 100)
