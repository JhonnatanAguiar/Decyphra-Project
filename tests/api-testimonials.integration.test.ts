...
describe('GET /api/v1/testimonials', () => {
  ...
  it('should support pagination with page and limit query params', async () => {
    const res = await fetch(`http://127.0.0.1:${port}/api/v1/testimonials?page=1&limit=5`)
    expect(res.status).toBe(200)
    
    const body = await res.json()
    expect(body.data.page).toBe(1)
    expect(body.data.limit).toBe(5)
    expect(body.data.testimonials.length).toBeLessThanOrEqual(5)
  })

  it('should support search query param', async () => {
    const res = await fetch(`http://127.0.0.1:${port}/api/v1/testimonials?search=test`)
    expect(res.status).toBe(200)
  ...
  it('should have total count matching or greater than testimonials array length', async () => {
    ...
    const body = await res.json()
    expect(body.data.total).toBeGreaterThanOrEqual(body.data.testimonials.length)
  })
})
