...
import type { Lead, Client } from '@prisma/client'
...
export async function listLeads(query: LeadListQuery): Promise<LeadListDTO> {
  ...
  return {
    leads: leads.map((lead: Lead & { convertedToClient: any; _count: { interactions: number } }) => ({
      ...lead,
...
export async function listClients(query: ClientListQuery): Promise<ClientListDTO> {
  ...
  return {
    clients: clients.map((client: Client & { convertedFromLead: any; _count: { interactions: number } }) => ({
      ...client,
